...
class AuthManager:
    def __init__(self, api_id, api_hash, api_url="https://apibotmassive-production.up.railway.app/user/" , db_name="jwt.db"):
        self.api_id = api_id
    ...
    async def sign_in(self, phone_number, client):
        try:
            ...
            def make_request():
                ...
                session.trust_env = False  # Esto ignora variables de entorno relacionadas con proxy
                response = session.post(f"{self.api_url}", json=data)
                try:
